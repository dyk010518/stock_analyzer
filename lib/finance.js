...
const getQuarterlyRevenue = (IS, quarter, ccersion) => {
    const revenue = IS.quarterlyReports[quarter].totalRevenue;
    return revenue !== "None" ? Number(revenue)*ccersion : undefined;
}
...
const getQuarterlyFCFMargin = (IS, CF, quarter, ccersion) => {
    const quarterFCF = getQuarterFCF(CF, quarter, ccersion);
    const quarterRevenue = getQuarterlyRevenue(IS, quarter, ccersion);
...
const getQuarterFCF = (CF, quarter, ccersion) => {
    const { operatingCashflow, capitalExpenditures } = CF.quarterlyReports[quarter];
    const OpCF = operatingCashflow !== "None" ? Number(operatingCashflow)*ccersion : undefined;
    const CapEx = capitalExpenditures !== "None" ? Number(capitalExpenditures)*ccersion : 0;
...
// Main functions
export const getRevenueNumbers = (IS, ccersion) => {
    // Yearly revenue based on quarter data
    const yearOne = getQuarterData(IS, 0, 4, (q) => getQuarterlyRevenue(IS, q, ccersion), AggregateMethod.SUM);
    const yearTwo = getQuarterData(IS, 4, 8, (q) => getQuarterlyRevenue(IS, q, ccersion), AggregateMethod.SUM);
    const yearFour = getQuarterData(IS, 12, 16, (q) => getQuarterlyRevenue(IS, q, ccersion), AggregateMethod.SUM);
    const yearSix = getQuarterData(IS, 20, 24, (q) => getQuarterlyRevenue(IS, q, ccersion), AggregateMethod.SUM);
